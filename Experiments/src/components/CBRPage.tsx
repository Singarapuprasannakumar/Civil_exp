    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-soft space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">Proving Ring Constant (kg/div)</label>
            <input
              type="number"
              step="0.1"
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">Surcharge Weight (kg)</label>
            <input
              type="number"
              step="0.5"
              value={surchargeWeight}
              onChange={(e) => setSurchargeWeight(parseFloat(e.target.value) || 5.0)}
              className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-[11px] font-bold text-slate-900 dark:text-white outline-none focus:border-blue-600"
            />
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">Penetration Dial Least Count (mm/div)</label>
            <input
              type="number"
              step="0.001"
              value={leastCount}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">Dry Density (g/cc)</label>
            <input
              type="number"
              step="0.01"
              value={dryDensity}
              onChange={(e) => setDryDensity(parseFloat(e.target.value) || 1.85)}
              className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-[11px] font-bold text-emerald-600 outline-none focus:border-blue-600"
            />
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">Moisture Content (%)</label>
            <input
              type="number"
              step="0.1"
              value={moistureContent}
              onChange={(e) => setMoistureContent(parseFloat(e.target.value) || 12.5)}
              className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-[11px] font-bold text-amber-600 outline-none focus:border-blue-600"
            />
          </div>
        </div>
      </div>
                ...
                <tr key={obs.sNo} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="p-2">
                    <input
                      ...
                      className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1 text-[11px] font-semibold text-blue-600 outline-none focus:border-blue-600 w-full shadow-inner"
                    />
                  </td>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-soft space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-[11px]">
          <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
            ...
            <code className="text-[11px] font-bold text-blue-600 font-mono block mt-1">= Dial Reading × Least Count</code>
          </div>

          <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
            <span className="font-semibold text-slate-500 block text-[11px]">Load (kg)</span>
            <code className="text-[11px] font-bold text-blue-600 font-mono block mt-1">= PRR × Ring Constant</code>
          </div>

          <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
            <span className="font-semibold text-slate-500 block text-[11px]">CBR Percentage</span>
            <code className="text-[11px] font-bold text-emerald-600 font-mono block mt-1">= (Measured / Standard) × 100</code>
          </div>

          <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
            <span className="font-semibold text-slate-500 block text-[11px]">Standard Loads</span>
            <code className="text-[11px] font-bold text-emerald-600 font-mono block mt-1">2.5mm: 1370kg | 5.0mm: 2055kg</code>
          </div>
        </div>
      </div>
      ...
      {!testValid && (
        <div className="bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-900 rounded-xl p-5 shadow-soft space-y-3">
          <div className="flex items-center gap-3 text-amber-700 dark:text-amber-400">
            ...
            <div>
              <h4 className="text-[11px] font-bold">IS 2720 Part 16 Warning: CBR 5.0 mm ({finalCbr50.toFixed(2)}%) &gt; CBR 2.5 mm ({finalCbr25.toFixed(2)}%)</h4>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-soft space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          ...
          <div className="md:col-span-6 grid grid-cols-2 gap-3 text-[11px] font-mono">
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
              <span className="text-[11px] text-slate-400 block">Load at 2.5 mm</span>
              <span className="font-bold text-blue-600">{load25.toFixed(2)} kg</span>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
              <span className="text-[11px] text-slate-400 block">Load at 5.0 mm</span>
              <span className="font-bold text-blue-600">{load50.toFixed(2)} kg</span>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
              <span className="text-[11px] text-slate-400 block">CBR at 2.5 mm</span>
              <span className="font-bold text-emerald-600">{finalCbr25.toFixed(2)} %</span>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
              <span className="text-[11px] text-slate-400 block">CBR at 5.0 mm</span>
